...
object NeoforgeConditionExtender {
    ...
    @JvmStatic
    fun extend(holder: IReadOnlyConditionHolder, vararg conditions: ICondition): Array<ICondition> {
        val json = JsonObject().also { holder.encode(it) }
        val encoded = GsonHelper.getAsJsonArray(json, "neoforge:conditions", JsonArray())
        val decoded = ICondition.LIST_CODEC.parse(JsonOps.INSTANCE, encoded)
            .result()
            .orElseGet(::emptyList)
